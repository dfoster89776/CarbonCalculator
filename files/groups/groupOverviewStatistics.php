<?php
	...
	//ENERGY STATISTICS
	$data['currentEnergy'] = $carbon->groupEnergyCarbonThisMonth($_SESSION['group_number']);
	$data['previousEnergy'] = $carbon->groupEnergyCarbonLastMonth($_SESSION['group_number']);
	@$data['percentageEnergy'] = round(($data['previousEnergy'] - $data['currentEnergy'])/$data['previousEnergy']*100);
	
	//TRANSPORT STATISTICS
	$data['currentTransport'] = $carbon->groupTransportCarbonThisMonth($_SESSION['group_number']);
	$data['previousTransport'] = $carbon->groupTransportCarbonLastMonth($_SESSION['group_number']);
	@$data['percentageTransport'] =  round(($data['previousTransport'] - $data['currentTransport'])/$data['previousTransport']*100);

	//LIFESTYLE STATISTICS
	$data['currentLifestyle'] = $carbon->groupLifestyleCarbonThisMonth($_SESSION['group_number']);
	$data['previousLifestyle'] = $carbon->groupLifestyleCarbonLastMonth($_SESSION['group_number']);
	@$data['percentageLifestyle'] =  round(($data['previousLifestyle'] - $data['currentLifestyle'])/$data['previousLifestyle']*100);
	$data['currentLifestyleOffset'] = $carbon->groupLifestyleCarbonOffsetThisMonth($_SESSION['group_number']);
	$data['previousLifestyleOffset'] = $carbon->groupLifestyleCarbonOffsetLastMonth($_SESSION['group_number']);
